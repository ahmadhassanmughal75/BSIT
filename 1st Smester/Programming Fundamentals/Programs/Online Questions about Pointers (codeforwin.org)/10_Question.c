// Q # 10 :: Write a C program to add two matrix using pointers.
...
int main() {
	...
	int sumMatrix[r][c], *fsmtr_ptr = sumMatrix;
	...
	for (i = 0; i < (r*c); i++) {
		*(fsmtr_ptr + i) = *(mtr1_ptr + i) + *(mtr2_ptr + i);
	}
	

	
	printf("\n\n\n:: Sumed Matrix ::"); // Printing sumed matrix (result)
	
	for (i = 0; i < (r*c); i++) {
		if(i % c == 0) {
			printf("\n\n");
		}
		printf("  %d  ", *(fsmtr_ptr + i));
	}
